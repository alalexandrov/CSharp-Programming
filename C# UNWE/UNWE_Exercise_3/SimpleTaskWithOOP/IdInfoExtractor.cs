...
{
    class IdInfoExtractor:Task
    {
        private string HumanID = "";

        public IdInfoExtractor(string ID)
        {
            this.HumanID = ID;
            this.TaskName = ID;
        }
        ...
        public override void ExecuteTask()
        {
            try
            {
                if (IsValidID(this.HumanID))
                {
                    string IdInfo = ExtractInfo(this.HumanID);
                    ShowResult(IdInfo);
                }
            }
        ...
        private static string ExtractInfo(string ID)
        {
            ...
            IdInfo.AppendFormat("Days to next Birthday: {0}", DaysToBirthDay);
